...
public class ProductDaoImpl implements ProductDao {
	...
	public List<ProductVO> betweenListProduct(String cid, int price1, int price2) throws SQLException {
		conn = ConnectionUtil.getConnectionUtil().getConnection();
		String sql = " SELECT pid, pname, pprice, pcontent, pimgurl, plikecount, cid FROM bo.PRODUCT WHERE CID LIKE ? AND PPRICE BETWEEN ? AND ? ";
		pstmt = conn.prepareStatement(sql);
		pstmt.setString(1, "%" + cid + "%");
		pstmt.setInt(2, price1);
		pstmt.setInt(3, price2);
		rs = pstmt.executeQuery();
		List<ProductVO> productList = new ArrayList<ProductVO>();
		if (rs != null) {
			while (rs.next()) {
	...
	public List<ProductVO> ListProductLowPrice(String cid) throws SQLException {
		conn = ConnectionUtil.getConnectionUtil().getConnection();
		String sql = " SELECT pid, pname, pprice, pcontent, pimgurl, plikecount, cid FROM bo.PRODUCT WHERE CID LIKE ? AND PPRICE <= 30000 ";
		pstmt = conn.prepareStatement(sql);
		pstmt.setString(1, "%" + cid + "%");
		rs = pstmt.executeQuery();
		List<ProductVO> productList = new ArrayList<ProductVO>();
	...
	public List<ProductVO> ListProductHighPrice(String cid) throws SQLException {
		conn = ConnectionUtil.getConnectionUtil().getConnection();
		String sql = " SELECT pid, pname, pprice, pcontent, pimgurl, plikecount, cid FROM bo.PRODUCT WHERE CID LIKE ? AND PPRICE >= 100000 ";
		pstmt = conn.prepareStatement(sql);
		pstmt.setString(1, "%" + cid + "%");
		rs = pstmt.executeQuery();
		List<ProductVO> productList = new ArrayList<ProductVO>();
