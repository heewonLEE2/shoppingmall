...
public class AiRecommendServer implements HttpHandler {
	...
	public void handle(HttpExchange exchange) throws IOException {
		...
		if ("POST".equalsIgnoreCase(exchange.getRequestMethod())) {
			...
			// ai 추천기능에서 사용자가 작성한 정보를 DTO 객체에 맵핑
			...
			try {
				if (aiDTO.getBudget().contains("~")) { // ~ 포함되어있으면 ~ 기준으로 잘라서 배열에 저장
					...
					// 예산 1 ~ 예산2 사이에 있는 상품을 가져오는 Service 이용
					int listSize = productService.betweenListProduct(aiDTO.getCid(), Integer.parseInt(aiDTObudget[0]),
							Integer.parseInt(aiDTObudget[1])).size();
					int randomNumber = random.nextInt(listSize); // 해당 상품을 랜덤으로 가져온다.
					response = gson.toJson(productService.betweenListProduct(aiDTO.getCid(),
							Integer.parseInt(aiDTObudget[0]), Integer.parseInt(aiDTObudget[1])).get(randomNumber));
