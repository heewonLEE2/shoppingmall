...
public class UserLoginCheckServer implements HttpHandler {
        ...
        if ("POST".equalsIgnoreCase(exchange.getRequestMethod())) {
            ...
            try {
                ...
                if (user == null || user.getUser_id() == null) {
                    ...
                    // 암호화된 비밀번호 복호화
                    String decryptedPassword = decryptRSA(loginDTO.getUser_pass());

                    // bcrypt 비교 (복호화된 평문 비밀번호, 저장된 bcrypt 해시)
                    if (BCrypt.checkpw(decryptedPassword, user.getUpass())) {
